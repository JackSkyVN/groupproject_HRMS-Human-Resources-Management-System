...
class SalaryAdjustment(Base):
    """
    Model for Salary Increase/Decrease Requests.
    Tracks the lifecycle of a salary adjustment from request to approval.
    """
    ...
    requester_id = Column(Integer, ForeignKey("employees.employee_id"), nullable=False, index=True)
    ...
    approved_by = Column(Integer, ForeignKey("employees.employee_id"), nullable=True, index=True)
